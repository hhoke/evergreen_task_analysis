...
class TaskTimes:
    ...
    def ingest_json(self,in_json):
        ...
        print('display')
        for task in display_task_ids:
            print(task)
        print('bad time')
        for task in bad_time_ids:
            print(task)
        for _id in bad_ids:
            logging.debug(tasks[_id])
            del tasks[_id]
        for _id in tasks:
            if 'depends_on' in tasks[_id]:
                to_delete = []
                for dep in tasks[_id]['depends_on']:
                    if dep['_id'] in display_task_ids:
                        to_delete.append(dep)
                for display_dep in to_delete:
                    tasks[_id]['depends_on'].remove(display_dep)
    ...
    def get_tasks(self, adhoc_screen=None, mode='polite_merge'):
        ...
        for _id in self.tasks:
            verbose = False
            task = self.tasks[_id] 
            if task == 'mongodb_mongo_master_enterprise_rhel_62_64_bit_dynamic_required_sharding_auth_audit_gen_a013b58aa9770fe637518fe896393546aaed294e_20_10_14_20_27_39':
                verbose = True
            invalid_field = False
            if screen:
                for field in screen:
                    if field not in task:
                        invalid_field = True
                        if verbose:
                            print('field {} not in task'.format(field))
                        break
                    elif screen[field]:
                        if task[field] not in screen[field]:
                            invalid_field = True
                            if verbose:
                                print('field {} invalid {}'.format(field, task[field]))
                            break
